...
<div id="Container" class="container-fluid cars-list">
    <div class="row">
        ...
        <div class=" col-md-4 col-sm-6 mix @foreach($car_categories as $c) category-{{ $c }} @endforeach" data-my-order="37" style="display: inline-block;" data-bound=""> {{--col-lg-3--}}
            ...
            <div class="mask">
                <p class="car_model">{{ session()->get('locale') == "ru" ? $car->brand : $car->brand_en }}</p>
                ...
                <p class="car_price"><span class="yellow">{{ __('С водителем') }}:</span> {{ session()->get('locale') == "ru" ? $car->price_with_driver : $car->price_with_driver_en }} {{ session()->get('locale') == "ru" ? __('грн') : '' }} / {{ __('час') }}</p>
                @if($car->transfer && $car->transfer > 0)
                    <p class="filling_time"><span class="yellow">{{ __('Трансфер') }}:</span> {{ session()->get('locale') == "ru" ? $car->transfer : $car->transfer_en }}</p>
                @endif
                @if($car->car_without_driver && isset(json_decode($car->rate_without_driver, true)[0]))
                    <p class="filling_time"><span class="yellow">{{ __('Без водителя') }}:</span> {{ json_decode($car->rate_without_driver, true)[0]['price'] }} / {{ __('сутки') }}</p>
                @endif
                <!-- <p class="out">Выезд за Киев- 50 грн/км.</p> -->
                <div class="but_wrap">
                    <a class="btn-order omore" href="{{ url(ucfirst(\App\Http\Middleware\Language::getLocale()).'/'.$car->url) }}">{{ __('подробнее') }}</a>
                </div>
            </div>
        </div>
